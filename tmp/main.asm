...
;	main.c: 124: uint8_t timing = get_timing(); // Get dip switch settings
	call	_get_timing
	ld	(0x01, sp), a
;	main.c: 126: while(read_pin('C', (1<<7), 1)); // Hold countdown until launch event detected (pin c7 DCed during launch)
00101$:
	push	#0x01
	ldw	x, #0x0080
	ld	a, #0x43
...
;	main.c: 129: while(1){
	clr	(0x04, sp)
00110$:
;	main.c: 130: if(elapsed >= (timing*10)){ // Check if timer has gone off
	ld	a, (0x01, sp)
	clrw	x
	ld	xl, a
	pushw	x
	...
	cpw	x, (0x02, sp)
	jrslt	00108$
;	main.c: 131: tog_pin('D', (1<<4)); // Activate ignitor
	push	#0x10
	ld	a, #0x44
	call	_tog_pin
;	main.c: 132: delay_ms(5000);  // Wait 5 seconds
	ldw	x, #0x1388
	call	_delay_ms
;	main.c: 133: tog_pin('D', (1<<4)); // Deactivate to avoid damage to lipos
	push	#0x10
	ld	a, #0x44
	call	_tog_pin
;	main.c: 134: while(1); // Program finished, 
00105$:
	jra	00105$
00108$:
;	main.c: 136: delay_ms(100); // If timer still going wait the 100 mills
	ldw	x, #0x0064
	call	_delay_ms
;	main.c: 137: elapsed++; // Another 100 mills has elapsed
	inc	(0x04, sp)
;	main.c: 138: tog_pin('B', (1 << 5)); // Blink light for ease of use
	push	#0x20
	ld	a, #0x42
	call	_tog_pin
	jra	00110$
